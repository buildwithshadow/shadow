...
- **One pinned block.** Each run reads the latest block once and makes every state read at that block number. It lists every line from `LineOpened` events: from `--index` up to its checkpoint when that block is still canonical, then a chunked log scan (5,000 blocks per request) from the checkpoint to the pinned block, so a line opened after the last index run is still checked. A checkpoint that was reorganized away, or that the RPC's head has not reached yet (an RPC lagging the one the index was built from), is described in `discovery.index.note` and the whole range is scanned instead. Without `--index` it scans from the deployment block on every run, so schedule it with the index. After its reads, it re-reads the pinned block's hash and fails if the block was reorganized during the run.
...
| `DISCOVERY_INCOMPLETE` | critical | Without `--line-id`: the lines found hold, in `availableReserve + principalOutstanding + recoveryAvailable`, other than `totalCommittedCapital`. A line is missing from the index and the scan, so none of its alerts (`DEFAULT_ELIGIBLE` included) can be raised, or the totals and the lines have diverged | Run `check` without `--index` (a full scan from the deployment block) and `reconcile`. If the full scan clears it, rebuild the index without `--resume` (`index --out index.json --manifest $M`). If it persists, `reconcile` fails as well: follow the accounting-divergence step of the incident procedure |
...
1. **Stop new risk.** An operator or the owner pauses:
   ...
   The owner tool accepts an operator's key for `pause` only. Pause openings as well when the cause is not understood.
...
3. **Contain, by cause** (threat model, incident boundaries):
   - suspected agent-key compromise: keep spends paused. `cancel-nonce` cannot contain it: it needs the agent's own key, and whoever holds that key can sign intents with fresh nonces, against the current terms too. Before any unpause, the sponsor contains each of the agent's lines:
     - an `OPEN` (debt-free) line: close it (`float-mainnet-sponsor.mjs close --line-id <line>`). Every intent on a `CLOSED` line reverts, and the reserve returns to the sponsor, who can open a new line for a new agent key;
     - a `DRAWN` line: deactivate every provider policy on it (`set-provider-policy --line-id <line> --provider <addr> --inactive`, once per provider from its `ProviderPolicySet` events). No spend executes while the line is `DRAWN` (`InvalidState`), but a repayment reopens it; afterwards an intent to an inactive provider pays nothing (`SpendBlocked(PROVIDER_NOT_ALLOWED)`), even one signed afresh. Close the line once it is repaid;
...
- **`obligationsEqualLines` or `committedCapitalEqualsLines` FAIL.** Either a line is missing from the list (compare `discovery.lines` with a run without `--index`, which scans from the deployment block), or the totals and the lines have diverged (spec `STATE-01`). If a full scan still fails, run the incident procedure with both pauses. The output's `lines` table gives each line's fields for tracing.
- **`linesMatchReserveCap` FAIL.** Its detail names each line and the identity it breaks. A line's amounts no longer add up to what its sponsor committed: reserve moved between lines or out of one (`CAP-01`, spec `STATE-01`), or, on a `DEFAULTED` line, a `SponsorClaimed` event is missing from the index (compare with a run without `--index`). If a full scan still fails, run the incident procedure with both pauses.
...
- two lines, one listed from an index and one from the scan past its checkpoint; every line, a single `--line-id`, a 60-day horizon raising `LINE_EXPIRY_SOON` and `POLICY_EXPIRY_SOON`;
...
- an index that lost the due line's `LineOpened`: `check` raises `DISCOVERY_INCOMPLETE` (exit 1) instead of the `DEFAULT_ELIGIBLE` it cannot see, `reconcile` exits 1, and `--line-id` skips the guard;
