...
- **Qualifying refusal.** Pre-agree a deliberate refusal while the line is open. Either make a real request to a provider endpoint outside the sponsor's policy (`ENDPOINT_NOT_ALLOWED`), or make a purchase above a limit (for example principal above the remaining reserve). The participant tools refuse a purchase they predict the contract would record as blocked. They send it only with an explicit `--allow-block` on `build`, `sign` and `submit` ([participant tools](SHADOW_FLOAT_MAINNET_PARTICIPANT_TOOLS.md)), so an accidental refusal is not produced through the tools. Record the digest, the reason and the zero transfer. Avoid `DAILY_SPEND_CAP`: a same-day retry can trigger it by accident, and that block uses up a signed intent. Log any unplanned block, but do not count it as the qualifying refusal.
